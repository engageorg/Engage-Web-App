...
export const redrawTextBoundingBox = (element) => {
    ...
    const metrics = measureText(element.originalText, getFontString(element), maxWidth);
...
export const bindTextToShapeAfterDuplication = (sceneElements, oldElements, oldIdToDuplicatedId) => {
    ...
    oldElements.forEach((element) => {
        ...
        if (boundTextElementId) {
            ...
            mutateElement(sceneElementMap.get(newElementId), {
                boundElements: element.boundElements?.concat({
                    type: "text",
                    id: newTextElementId,
                }),
            });
            mutateElement(sceneElementMap.get(newTextElementId), {
                containerId: newElementId,
            });
        }
    });
};
export const handleBindTextResize = (elements, transformHandleType) => {
            ...
            if (textElement && textElement.text) {
                ...
                if (transformHandleType !== "n" && transformHandleType !== "s") {
                    ...
                    if (text) {
                        ...
                        const diff = Math.abs(element.width - textElement.width + PADDING * 2);
                        if (diff >= minCharWidthTillNow) {
                            text = wrapText(textElement.originalText, getFontString(textElement), element.width);
                        }
                    }
                    const dimensions = measureText(text, getFontString(textElement), element.width);
                ...
                if (nextHeight > element.height - PADDING * 2) {
                    ...
                    const updatedY = transformHandleType === "ne" ||
...
export const measureText = (text, font, maxWidth) => {
    text = text
        .split("\n")
...
export const getApproxLineHeight = (font) => {
...
export const wrapText = (text, font, containerWidth) => {
...
export const charWidth = (() => {
    ...
    const updateCache = (char, font) => {
        const ascii = char.charCodeAt(0);
        if (!cachedCharWidth[font][ascii]) {
            cachedCharWidth[font][ascii] = calculate(char, font);
        }
    };
    const clearCacheforFont = (font) => {
        cachedCharWidth[font] = [];
    };
    const getCache = (font) => {
...
export const getApproxMinLineWidth = (font) => {
    return measureText(DUMMY_TEXT.split("").join("\n"), font).width + PADDING * 2;
};
export const getApproxMinLineHeight = (font) => {
