...
import { actionAddToLibrary, actionBringForward, actionBringToFront, actionCopy, actionCopyAsPng, actionCopyAsSvg, actionCopyStyles, actionCut, actionDeleteSelected, actionDuplicateSelection, actionFinalize, actionFlipHorizontal, actionFlipVertical, actionGroup, actionPasteStyles, actionSelectAll, actionSendBackward, actionSendToBack, actionToggleGridMode, actionToggleStats, actionToggleZenMode, actionUngroup, } from "../actions";
...
import { APP_NAME, CURSOR_TYPE, DEFAULT_MAX_IMAGE_WIDTH_OR_HEIGHT, DEFAULT_UI_OPTIONS, DEFAULT_VERTICAL_ALIGN, DRAGGING_THRESHOLD, ELEMENT_SHIFT_TRANSLATE_AMOUNT, ELEMENT_TRANSLATE_AMOUNT, ENV, EVENT, GRID_SIZE, IMAGE_RENDER_TIMEOUT, LINE_CONFIRM_THRESHOLD, MAX_ALLOWED_FILE_BYTES, MIME_TYPES, MQ_MAX_HEIGHT_LANDSCAPE, MQ_MAX_WIDTH_LANDSCAPE, MQ_MAX_WIDTH_PORTRAIT, POINTER_BUTTON, SCROLL_TIMEOUT, TAP_TWICE_TIMEOUT, TEXT_TO_CENTER_SNAP_THRESHOLD, THEME, TOUCH_CTX_MENU_TIMEOUT, URL_HASH_KEYS, URL_QUERY_KEYS, ZOOM_STEP, } from "../constants";
import { loadFromBlob } from "../data";
import { isValidLibrary } from "../data/json";
import Library from "../data/library";
import { restore, restoreElements, restoreLibraryItems } from "../data/restore";
import { dragNewElement, dragSelectedElements, duplicateElement, getCommonBounds, getCursorForResizingElement, getDragOffsetXY, getElementWithTransformHandleType, getNormalizedDimensions, getPerfectElementSize, getResizeArrowDirection, getResizeOffsetXY, getTransformHandleTypeFromCoords, hitTest, isHittingElementBoundingBoxWithoutHittingElement, isInvisiblySmallElement, isNonDeletedElement, isTextElement, newElement, newLinearElement, newTextElement, newImageElement, textWysiwyg, transformElements, updateTextElement, } from "../element";
...
import { bumpVersion, mutateElement, newElementWith, } from "../element/mutateElement";
import { deepCopyElement, newFreeDrawElement } from "../element/newElement";
import { hasBoundTextElement, isBindingElement, isBindingElementType, isImageElement, isInitializedImageElement, isLinearElement, isLinearElementType, } from "../element/typeChecks";
...
import { calculateScrollCenter, getElementContainingPosition, getElementsAtPosition, getElementsWithinSelection, getNormalizedZoom, getSelectedElements, hasBackground, isOverScrollBars, isSomeElementSelected, } from "../scene";
import Scene from "../scene/Scene";
import { getNewZoom } from "../scene/zoom";
import { findShapeByKey } from "../shapes";
import { debounce, distance, getFontString, getNearestScrollableContainer, isInputLike, isToolIcon, isWritableElement, resetCursor, resolvablePromise, sceneCoordsToViewportCoords, setCursor, setCursorForShape, tupleToCoors, viewportCoordsToSceneCoords, withBatchedUpdates, } from "../utils";
import ContextMenu from "./ContextMenu";
import LayerUI from "./LayerUI";
import { Stats } from "./Stats";
import { Toast } from "./Toast";
import { actionToggleViewMode } from "../actions/actionToggleViewMode";
import { dataURLToFile, generateIdFromFile, getDataURL, isSupportedImageFile, resizeImageFile, SVGStringToFile, } from "../data/blob";
...
import { bindTextToShapeAfterDuplication, getApproxMinLineHeight, getApproxMinLineWidth, getBoundTextElementId, } from "../element/textElement";
import { isHittingElementNotConsideringBoundingBox } from "../element/collision";
const IsMobileContext = React.createContext(false);
export const useIsMobile = () => useContext(IsMobileContext);
const ExcalidrawContainerContext = React.createContext({ container: null, id: null });
...
class App extends React.Component {
    constructor(props) {
        ...
        this.syncActionResult = withBatchedUpdates((actionResult) => {
            ...
            if (actionResult.elements) {
                actionResult.elements.forEach((element) => {
                    if (this.state.editingElement?.id === element.id &&
        ...
        this.initializeScene = async () => {
            if ("launchQueue" in window && "LaunchParams" in window) {
                window.launchQueue.setConsumer(async (launchParams) => {
                    ...
                    loadFromBlob(blob, this.state, this.scene.getElementsIncludingDeleted())
                        .then((scene) => {
            ...
            try {
                ...
                if (initialData?.libraryItems) {
                    this.libraryItemsFromStorage = restoreLibraryItems(initialData.libraryItems, "unpublished");
                }
            }
            catch (error) {
                console.error(error);
        ...
        this.pasteFromClipboard = withBatchedUpdates(async (event) => {
            ...
            if (isSupportedImageFile(file)) {
                const { x: sceneX, y: sceneY } = viewportCoordsToSceneCoords({ clientX: cursorX, clientY: cursorY }, this.state);
        ...
        this.scrollToContent = (target = this.scene.getElements()) => {
            this.setState({
                ...calculateScrollCenter(Array.isArray(target) ? target : [target], this.state, this.canvas),
            });
        };
        this.clearToast = () => {
        ...
        this.onGestureChange = withBatchedUpdates((event) => {
            ...
            if (initialScale) {
                this.setState(({ zoom, offsetLeft, offsetTop }) => ({
                    zoom: getNewZoom(getNormalizedZoom(initialScale * event.scale), zoom, { left: offsetLeft, top: offsetTop }, { x: cursorX, y: cursorY }),
                }));
            }
        });
        ...
        this.startTextEditing = ({ sceneX, sceneY, shouldBind, insertAtParentCenter = true, }) => {
            ...
            const container = shouldBind || parentCenterPosition
                ? getElementContainingPosition(this.scene.getElements(), sceneX, sceneY, "text")
                : null;
        ...
        this.handleCanvasPointerMove = (event) => {
            ...
            if (gesture.pointers.size === 2 &&
                ...
                this.setState(({ zoom, scrollX, scrollY, offsetLeft, offsetTop }) => ({
                    ...
                    zoom: getNewZoom(getNormalizedZoom(initialScale * scaleFactor), zoom, { left: offsetLeft, top: offsetTop }, center),
            ...
            if (this.state.multiElement) {
                ...
                if (lastPoint === lastCommittedPoint) {
                    ...
                    lastCommittedPoint &&
                    distance2d(scenePointerX - rx, scenePointerY - ry, lastCommittedPoint[0], lastCommittedPoint[1]) < LINE_CONFIRM_THRESHOLD) {
                    setCursor(this.canvas, CURSOR_TYPE.POINTER);
                    mutateElement(multiElement, {
            ...
            if (this.state.elementType === "text") {
                ...
                const element = LinearElementEditor.getElement(this.state.editingLinearElement.elementId);
                if (element &&
        ...
        this.handleSelectionOnPointerDown = (event, pointerDownState) => {
            if (this.state.elementType === "selection") {
                ...
                if (pointerDownState.resize.handleType) {
                    ...
                    if (this.state.editingLinearElement) {
                        const ret = LinearElementEditor.handlePointerDown(event, this.state, (appState) => this.setState(appState), this.history, pointerDownState.origin);
                        if (ret.hitElement) {
                    ...
                    if (pointerDownState.hit.element) {
                        pointerDownState.hit.hasHitElementInside =
                            isHittingElementNotConsideringBoundingBox(pointerDownState.hit.element, this.state, [pointerDownState.origin.x, pointerDownState.origin.y]);
                    }
        ...
        this.handleLinearElementOnPointerDown = (event, elementType, pointerDownState) => {
            if (this.state.multiElement) {
                ...
                if (multiElement.points.length > 1 &&
                    lastCommittedPoint &&
                    distance2d(pointerDownState.origin.x - rx, pointerDownState.origin.y - ry, lastCommittedPoint[0], lastCommittedPoint[1]) < LINE_CONFIRM_THRESHOLD) {
                    this.actionManager.executeAction(actionFinalize);
                    return;
                }
                this.setState((prevState) => ({
        ...
        this.initializeImage = async ({ imageFile, imageElement: _imageElement, showCursorImagePreview = false, }) => {
            ...
            const fileId = await (this.props.generateIdForFile?.(imageFile) || generateIdFromFile(imageFile));
            if (!fileId) {
        ...
        this.onImageAction = async ({ insertOnCanvasDirectly } = { insertOnCanvasDirectly: false }) => {
            try {
        ...
        this.initializeImageDimensions = (imageElement, forceNaturalSize = false) => {
            const image = isInitializedImageElement(imageElement) &&
                this.imageCache.get(imageElement.fileId)?.image;
            if (!image || image instanceof Promise) {
                if (imageElement.width < DRAGGING_THRESHOLD / this.state.zoom.value &&
        ...
        this.addNewImagesToImageCache = async (imageElements = getInitializedImageElements(this.scene.getElements()), files = this.files) => {
        ...
        this.handleAppOnDrop = async (event) => {
                ...
                if (isSupportedImageFile(file)) {
                    ...
                    if (file?.type === MIME_TYPES.png || file?.type === MIME_TYPES.svg) {
                        try {
                            ...
                            const scene = await loadFromBlob(file, this.state, this.scene.getElementsIncludingDeleted());
                            this.syncActionResult({
                                ...scene,
                                appState: {
                                    ...(scene.appState || this.state),
            ...
            const file = event.dataTransfer?.files[0];
            if (file?.type === MIME_TYPES.excalidrawlib ||
        ...
        this.maybeDragNewGenericElement = (pointerDownState, event) => {
            ...
            if (draggingElement.type === "selection") {
                dragNewElement(draggingElement, this.state.elementType, pointerDownState.origin.x, pointerDownState.origin.y, pointerCoords.x, pointerCoords.y, distance(pointerDownState.origin.x, pointerCoords.x), distance(pointerDownState.origin.y, pointerCoords.y), shouldMaintainAspectRatio(event), shouldResizeFromCenter(event));
            }
            else {
                const [gridX, gridY] = getGridPoint(pointerCoords.x, pointerCoords.y, this.state.gridSize);
                const image = isInitializedImageElement(draggingElement) &&
                    this.imageCache.get(draggingElement.fileId)?.image;
                const aspectRatio = image && !(image instanceof Promise)
                    ? image.width / image.height
                    : null;
                dragNewElement(draggingElement, this.state.elementType, pointerDownState.originInGrid.x, pointerDownState.originInGrid.y, gridX, gridY, distance(pointerDownState.originInGrid.x, gridX), distance(pointerDownState.originInGrid.y, gridY), isImageElement(draggingElement)
                    ? !shouldMaintainAspectRatio(event)
                    : shouldMaintainAspectRatio(event), shouldResizeFromCenter(event), aspectRatio);
        ...
        this._openContextMenu = ({ left, top, }, type) => {
            const maybeGroupAction = actionGroup.contextItemPredicate(this.actionManager.getElementsIncludingDeleted(), this.actionManager.getAppState());
            const maybeUngroupAction = actionUngroup.contextItemPredicate(this.actionManager.getElementsIncludingDeleted(), this.actionManager.getAppState());
            const maybeFlipHorizontal = actionFlipHorizontal.contextItemPredicate(this.actionManager.getElementsIncludingDeleted(), this.actionManager.getAppState());
            const maybeFlipVertical = actionFlipVertical.contextItemPredicate(this.actionManager.getElementsIncludingDeleted(), this.actionManager.getAppState());
        ...
        this.handleWheel = withBatchedUpdates((event) => {
            ...
            if (event.metaKey || event.ctrlKey) {
                ...
                this.setState(({ zoom, offsetLeft, offsetTop }) => ({
                    zoom: getNewZoom(getNormalizedZoom(newZoom), zoom, { left: offsetLeft, top: offsetTop }, {
                        x: cursorX,
                        y: cursorY,
                    }),
                    selectedElementIds: {},
        ...
        this.id = nanoid();
        if (excalidrawRef) {
            const readyPromise = ("current" in excalidrawRef && excalidrawRef.current?.readyPromise) ||
                resolvablePromise();
            const api = {
                ready: true,
                readyPromise,
    ...
    renderCanvas() {
        ...
        if (viewModeEnabled) {
            return (<canvas className="excalidraw__canvas" style={{
                    width: canvasDOMWidth,
                    height: canvasDOMHeight,
                    cursor: CURSOR_TYPE.GRAB,
                }} width={canvasWidth} height={canvasHeight} ref={this.handleCanvasRef} onContextMenu={this.handleCanvasContextMenu} onPointerMove={this.handleCanvasPointerMove} onPointerUp={this.removePointer} onPointerCancel={this.removePointer} onTouchMove={this.handleTouchMove} onPointerDown={this.handleCanvasPointerDown}>
        ...
        return (<canvas className="excalidraw__canvas" style={{
                width: canvasDOMWidth,
                height: canvasDOMHeight,
            }} width={canvasWidth} height={canvasHeight} ref={this.handleCanvasRef} onContextMenu={this.handleCanvasContextMenu} onPointerDown={this.handleCanvasPointerDown} onDoubleClick={this.handleCanvasDoubleClick} onPointerMove={this.handleCanvasPointerMove} onPointerUp={this.removePointer} onPointerCancel={this.removePointer} onTouchMove={this.handleTouchMove}>
        {t("labels.drawingCanvas")}
      </canvas>);
    }
    render() {
        ...
        return (<div className={clsx("excalidraw excalidraw-container", {
                "excalidraw--view-mode": viewModeEnabled,
                "excalidraw--mobile": this.isMobile,
            })} ref={this.excalidrawContainerRef} onDrop={this.handleAppOnDrop} tabIndex={0} onKeyDown={this.props.handleKeyboardGlobally ? undefined : this.onKeyDown}>
        <ExcalidrawContainerContext.Provider value={this.excalidrawContainerValue}>
          <IsMobileContext.Provider value={this.isMobile}>
            <LayerUI canvas={this.canvas} appState={this.state} files={this.files} setAppState={this.setAppState} actionManager={this.actionManager} elements={this.scene.getElements()} onCollabButtonClick={onCollabButtonClick} onLockToggle={this.toggleLock} onInsertElements={(elements) => this.addElementsFromPasteOrLibrary({
                elements,
                position: "center",
                files: null,
            })} zenModeEnabled={zenModeEnabled} toggleZenMode={this.toggleZenMode} langCode={getLanguage().code} isCollaborating={this.props.isCollaborating || false} renderTopRightUI={renderTopRightUI} renderCustomFooter={renderFooter} viewModeEnabled={viewModeEnabled} showExitZenModeBtn={typeof this.props?.zenModeEnabled === "undefined" &&
                zenModeEnabled} showThemeBtn={typeof this.props?.theme === "undefined" &&
                this.props.UIOptions.canvasActions.theme} libraryReturnUrl={this.props.libraryReturnUrl} UIOptions={this.props.UIOptions} focusContainer={this.focusContainer} library={this.library} id={this.id} onImageAction={this.onImageAction}/>
            <div className="excalidraw-textEditorContainer"/>
            <div className="excalidraw-contextMenuContainer"/>
            {this.state.showStats && (<Stats appState={this.state} setAppState={this.setAppState} elements={this.scene.getElements()} onClose={this.toggleStats} renderCustomStats={renderCustomStats}/>)}
            {this.state.toastMessage !== null && (<Toast message={this.state.toastMessage} clearToast={this.clearToast}/>)}
    ...
    async componentDidMount() {
        ...
        if ("ResizeObserver" in window && this.excalidrawContainerRef?.current) {
            ...
            const mediaQuery = window.matchMedia(`(max-width: ${MQ_MAX_WIDTH_PORTRAIT}px), (max-height: ${MQ_MAX_HEIGHT_LANDSCAPE}px) and (max-width: ${MQ_MAX_WIDTH_LANDSCAPE}px)`);
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        const { atLeastOneVisibleElement, scrollBars } = renderScene(renderingElements, this.state, this.state.selectionElement, window.devicePixelRatio, this.rc, this.canvas, {
    ...
    addTextFromPaste(text) {
        const { x, y } = viewportCoordsToSceneCoords({ clientX: cursorX, clientY: cursorY }, this.state);
    ...
    getElementAtPosition(x, y, opts) {
        const allHitElements = this.getElementsAtPosition(x, y, opts?.includeBoundTextElement);
        if (allHitElements.length > 1) {
            ...
            return isHittingElementBoundingBoxWithoutHittingElement(elementWithHighestZIndex, this.state, x, y)
    ...
    onPointerMoveFromPointerDownHandler(pointerDownState) {
        return withBatchedUpdates((event) => {
            ...
            if (this.state.editingLinearElement) {
                const didDrag = LinearElementEditor.handlePointDragging(this.state, (appState) => this.setState(appState), pointerCoords.x, pointerCoords.y, (element, pointsSceneCoords) => {
            ...
            if ((hasHitASelectedElement ||
                ...
                if (selectedElements.length > 0 && !pointerDownState.withCmdOrCtrl) {
                    ...
                    dragSelectedElements(pointerDownState, selectedElements, dragX, dragY, lockDirection, dragDistanceX, dragDistanceY);
                    this.maybeSuggestBindingForAll(selectedElements);
                    ...
                    if (event.altKey && !pointerDownState.hit.hasBeenDuplicated) {
                        ...
                        const selectedElementIds = getSelectedElements(elements, this.state, true).map((element) => element.id);
                        for (const element of elements) {
            ...
            if (draggingElement.type === "freedraw") {
                ...
                if (shouldRotateWithDiscreteAngle(event) && points.length === 2) {
                    ({ width: dx, height: dy } = getPerfectElementSize(this.state.elementType, dx, dy));
                }
                if (points.length === 1) {
    ...
    onPointerUpFromPointerDownHandler(pointerDownState) {
        return withBatchedUpdates((childEvent) => {
                    ...
                    if (this.state.selectedElementIds[hitElement.id]) {
                        if (isSelectedViaGroup(this.state, hitElement)) {
                            ...
                            const idsOfSelectedElementsThatAreInGroups = hitElement.groupIds
                                .flatMap((groupId) => getElementsInGroup(this.scene.getElements(), groupId))
            ...
            if (!this.state.editingLinearElement &&
                ...
                ((hitElement &&
                    isHittingElementBoundingBoxWithoutHittingElement(hitElement, this.state, pointerDownState.origin.x, pointerDownState.origin.y)) ||
                    (!hitElement &&
                        pointerDownState.hit.hasHitCommonBoundingBoxOfSelectedElements))) {
            ...
            if (pointerDownState.drag.hasOccurred || isResizing || isRotating) {
                (isBindingEnabled(this.state)
                    ...
                    : unbindLinearElements)(getSelectedElements(this.scene.getElements(), this.state));
            }
    ...
    getTextWysiwygSnappedToCenterPosition(x, y, appState, canvas, scale) {
        const elementClickedInside = getElementContainingPosition(this.scene
            .getElementsIncludingDeleted()
            .filter((element) => !isTextElement(element)), x, y);
        if (elementClickedInside) {
            ...
            if (isSnappedToCenter) {
                const { x: viewportX, y: viewportY } = sceneCoordsToViewportCoords({ sceneX: elementCenterX, sceneY: elementCenterY }, appState);
    ...
    async updateLanguage() {
        const currentLang = languages.find((lang) => lang.code === this.props.langCode) ||
            defaultLang;
