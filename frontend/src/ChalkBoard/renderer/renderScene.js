...
const renderLinearPointHandles = (context, appState, renderConfig, element) => {
    ...
    LinearElementEditor.getPointsGlobalCoordinates(element).forEach((point, idx) => {
        ...
        fillCircle(context, point[0], point[1], POINT_HANDLE_SIZE / 2 / renderConfig.zoom.value);
    });
    context.restore();
};
export const renderScene = (elements, appState, selectionElement, scale, rc, canvas, renderConfig) => {
    if (canvas === null) {
    ...
    if (typeof renderConfig.viewBackgroundColor === "string") {
        const hasTransparence = renderConfig.viewBackgroundColor === "transparent" ||
    ...
    if (renderGrid && appState.gridSize) {
        strokeGrid(context, appState.gridSize, -Math.ceil(zoomTranslationX / renderConfig.zoom.value / appState.gridSize) *
            appState.gridSize +
            (renderConfig.scrollX % appState.gridSize), -Math.ceil(zoomTranslationY / renderConfig.zoom.value / appState.gridSize) *
            appState.gridSize +
            (renderConfig.scrollY % appState.gridSize), normalizedCanvasWidth / renderConfig.zoom.value, normalizedCanvasHeight / renderConfig.zoom.value);
    }
    ...
    if (renderSelection &&
        ...
        if (locallySelectedElements.length === 1) {
            ...
            const transformHandles = getTransformHandles(locallySelectedElements[0], renderConfig.zoom, "mouse");
            ...
            const transformHandles = getTransformHandlesFromCoords([x1, y1, x2, y2], 0, renderConfig.zoom, "mouse", OMIT_SIDES_FOR_MULTIPLE_ELEMENTS);
...
const renderSelectionBorder = (context, renderConfig, elementProperties) => {
...
export const renderSceneToSvg = (elements, rsvg, svgRoot, files, { offsetX = 0, offsetY = 0, exportWithDarkMode = false, } = {}) => {
    if (!svgRoot) {
        return;
    }
    // render elements
    elements.forEach((element) => {
        if (!element.isDeleted) {
            try {
                renderElementToSvg(element, rsvg, svgRoot, files, element.x + offsetX, element.y + offsetY, exportWithDarkMode);
            }
            catch (error) {
                console.error(error);
            }
