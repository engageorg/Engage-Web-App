...
const parseId = (uid) => {
    const [, parent, id, version] = uid.match(/^(?:\((\^|\w+)\))?(\w+)(?::(\d+))?(?:\((\w+)\))?$/);
    return {
...
const test = (local, remote, target, bidirectional = true) => {
    const cache = {};
    ...
    const remoteReconciled = reconcileElements(_local, _remote, {});
    ...
    if (bidirectional) {
        try {
            expect(cleanElements(reconcileElements(cloneDeep(__local), cloneDeep(__remote), {}))).deep.equal(cleanElements(remoteReconciled), "local re-reconciliation");
        }
        catch (error) {
...
describe("elements reconciliation", () => {
    it("reconcileElements()", () => {
        ...
        test(["A", "B", "C", "D", "E", "F"], ["A", "B:2", "X", "E:2", "F", "Y"], ["A:L", "B:R", "X:R", "E:R", "F:L", "Y:R", "C:L", "D:L"]);
        // annotated elements
        // -------------------------------------------------------------------------
        ...
        test(["A", "B", "C", "D", "E"], ["(A)X", "(C)Y", "(D)Z"], ["A:L", "X:R", "B:L", "C:L", "Y:R", "D:L", "Z:R", "E:L"]);
        test(["X", "Y", "Z"], ["(^)A", "(A)B", "(B)C", "(C)X", "(X)D", "(D)Y", "(Y)Z"], ["A:R", "B:R", "C:R", "X:L", "D:R", "Y:L", "Z:L"]);
        test(["A", "B", "C", "D", "E"], ["(C)X", "(A)Y", "(D)E:1"], ["A:L", "B:L", "C:L", "X:R", "Y:R", "D:L", "E:R"]);
