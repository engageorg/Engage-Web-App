...
describe("resize rectangle ellipses and diamond elements", () => {
  ...
  `("resizes with handle $handle", async ({ handle, move, dimensions, topLeft }) => {
    ...
    ${"sw"} | ${[40, -20]}    | ${[80, 80]}   | ${[20, 0]}
  `("resizes with fixed side ratios on handle $handle", async ({ handle, move, dimensions, topLeft }) => {
        await render(<App />);
        const rectangle = UI.createElement("rectangle", elemData);
  ...
  `("Flips while resizing and keeping side ratios on handle $handle", async ({ handle, move, dimensions, topLeft }) => {
        await render(<App />);
        const rectangle = UI.createElement("rectangle", elemData);
        resize(rectangle, handle, move, { shift: true });
        const element = h.elements[0];
  ...
  `("Resizes from center, flips and keeps side rations on handle $handle", async ({ handle, move, dimensions, topLeft }) => {
        await render(<App />);
        const rectangle = UI.createElement("rectangle", elemData);
...
const resize = (element, handleDir, mouseMove, keyboardModifiers = {}) => {
    mouse.select(element);
    const handle = getTransformHandles(element, h.state.zoom, "mouse")[handleDir];
