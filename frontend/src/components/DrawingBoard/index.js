...
import { newElement, duplicateElement, resizeTest, isTextElement, textWysiwyg, getElementAbsoluteCoords } from "./element";
import { clearSelection, deleteSelectedElements, setSelection, isOverScrollBars, restoreFromLocalStorage, saveToLocalStorage, getElementAtPosition, createScene, getElementContainingPosition } from "./scene";
...
export function viewportCoordsToSceneCoords({ clientX, clientY }, { scrollX, scrollY }) {
...
export default class App extends React.Component {
    constructor(props) {
        ...
        this.copyToClipboard = () => {
            if (navigator.clipboard) {
                const text = JSON.stringify(elements.filter(element => element.isSelected));
        ...
        this.addElementsFromPaste = (paste) => {
            ...
            if (Array.isArray(parsedElements) &&
                ...
                const dx = this.state.cursorX -
                    this.state.scrollX -
                    CANVAS_WINDOW_OFFSET_LEFT -
                    elementsCenterX;
                const dy = this.state.cursorY -
                    this.state.scrollY -
    ...
    componentWillUnmount() {
        ...
        document.removeEventListener("mousemove", this.getCurrentCursorPosition, false);
        window.removeEventListener("resize", this.onResize, false);
    }
    render() {
        const canvasWidth = window.innerWidth - CANVAS_WINDOW_OFFSET_LEFT;
        const canvasHeight = window.innerHeight - CANVAS_WINDOW_OFFSET_TOP;
        return (<div className = "chalkboard_container" onCut={e => {
        ...
        <SidePanel actionManager={this.actionManager} syncActionResult={this.syncActionResult} appState={{ ...this.state }} elements={elements} onToolChange={value => {
        ...
        <canvas id="canvas" style={{
            width: canvasWidth,
            height: canvasHeight
        }} width={canvasWidth * window.devicePixelRatio} height={canvasHeight * window.devicePixelRatio} ref={canvas => {
            if (this.canvas === null) {
                this.canvas = canvas;
            ...
            if (canvas) {
                ...
                this.removeWheelEventListener = () => canvas.removeEventListener("wheel", this.handleWheel);
        ...
        onContextMenu={e => {
            ...
            ContextMenu.push({
                options: [
                    ...
                    ...this.actionManager.getContextMenuItems(elements, this.state, this.syncActionResult)
                ],
                top: e.clientY,
                left: e.clientX
            });
        ...
        onMouseDown={e => {
            ...
            const { isOverHorizontalScrollBar, isOverVerticalScrollBar } = isOverScrollBars(elements, e.clientX - CANVAS_WINDOW_OFFSET_LEFT, e.clientY - CANVAS_WINDOW_OFFSET_TOP, canvasWidth, canvasHeight, this.state.scrollX, this.state.scrollY);
            const { x, y } = viewportCoordsToSceneCoords(e, this.state);
            const element = newElement(this.state.elementType, x, y, this.state.currentItemStrokeColor, this.state.currentItemBackgroundColor, "hachure", 1, 1, 100);
            ...
            if (this.state.elementType === "selection") {
                const resizeElement = getElementWithResizeHandler(elements, { x, y }, this.state);
                this.setState({
                ...
                if (resizeElement) {
                    ...
                    const selected = getElementAtPosition(elements.filter(el => el.isSelected), x, y);
                        ...
                        if (e.altKey) {
                            elements = [
                                ...
                                ...elements.reduce((duplicates, element) => {
                                    if (element.isSelected) {
                                        duplicates = duplicates.concat(duplicateElement(element));
            ...
            if (isTextElement(element)) {
                ...
                let textY = e.clientY;
                if (!e.altKey) {
                    const snappedToCenterPosition = this.getTextWysiwygSnappedToCenterPosition(x, y);
            ...
            const onMouseMove = (e) => {
                ...
                if ((_a = hitElement) === null || _a === void 0 ? void 0 : _a.isSelected) {
        ...
        onDoubleClick = {e => {
            ...
            const element = newElement("text", x, y, this.state.currentItemStrokeColor, this.state.currentItemBackgroundColor, "hachure", 1, 1, 100);
            let initText = "";
            let textX = e.clientX;
            let textY = e.clientY;
            if (elementAtPosition && isTextElement(elementAtPosition)) {
                elements = elements.filter(element => element.id !== elementAtPosition.id);
                this.forceUpdate();
            ...
            textWysiwyg({
                ...
                onSubmit: text => {
                    addTextElement(element, text, element.font || this.state.currentItemFont);
                    elements = [...elements, { ...element, isSelected: true }];
                    this.setState({
                        draggingElement: null,
                        elementType: "selection"
    ...
    getTextWysiwygSnappedToCenterPosition(x, y) {
        ...
        if (elementClickedInside) {
            ...
            if (isSnappedToCenter) {
                const wysiwygX = this.state.scrollX +
                    elementClickedInside.x +
                    CANVAS_WINDOW_OFFSET_LEFT +
                    elementClickedInside.width / 2;
                const wysiwygY = this.state.scrollY +
